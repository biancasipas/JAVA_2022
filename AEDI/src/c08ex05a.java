...
public class c08ex05a {
	...
	for (int repeticao = 1; repeticao <= 10; repeticao++) {
		 voto = Integer.parseInt(JOptionPane.showInputDialog(ANSI_YELLOW+"Qual número de voto"));
		  switch(voto){
		   case 1:
 ...
 if (total<nulo){
     ...
     "Número de votos nulos maior que votos válidos, eleição encerrada",
     "ELEIÇÃO",
 ...
 for(int repeticao = 1; repeticao <= 10; repeticao++) {
	if(vota_primeiro>vota_segundo) {
	JOptionPane.showMessageDialog(null,candidato1+"Fulano venceu as eleições");
	}else if (vota_segundo>vota_primeiro){
	JOptionPane.showMessageDialog(null,candidato2+"Ciclano venceu as eleições");
	}else {
